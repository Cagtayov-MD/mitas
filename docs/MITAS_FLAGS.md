...
| `MITAS_CREDIT_TEXT_MODEL` | `gemma-4-31b-it-qat-vision:latest` | `scripts/credit_text_read.py:29` | Künye-metin ayıklayıcı (kredi-okuma) ollama modeli (multimodal; metin text-only ile BİREBİR aynı). | HAYIR | config | qwen3.6:35b→gemma-vision geçişi (Çağatay 2026-06-23); ~18GB VRAM. Geri dön: `=qwen3.6:35b-a3b`. |
...
| `MITAS_CREDIT_TEXT_MODEL` | `gemma-4-31b-it-qat-vision:latest` | Ayıklayıcı model seçimi | Görünmez; VRAM/hız etkisi büyük, start'ta override edilmiyor. (2026-06-23 qwen3.6:35b→gemma-vision multimodal) |
