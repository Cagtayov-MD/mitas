"""BoxMotionTrack test koşucusu.

Kullanım:
    python run_boxtracking_test.py
    python run_boxtracking_test.py --cases 1980_son_metro_end_credits__closing 2000_x_men_end_credits__closing

Çıktı: outputs/_boxtracking_test_<timestamp>/<film>__<seg>/
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="BoxMotionTrack test koşucusu")
    parser.add_argument(
        "--cases",
        nargs="*",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Her case için maksimum kare sayısı (test için kısalt)",
    )
    ...
    output_base = PROJECT_ROOT / "outputs" / f"_boxtracking_test_{ts}"
    output_base.mkdir(parents=True, exist_ok=True)
    print(f"[BoxTracking] Çıktı dizini: {output_base}")

    # --- PaddleOCR engine'i başlat (tek seferlik) ---
    print("[BoxTracking] PaddleOCR engine başlatılıyor...")
    t0 = perf_counter()
    try:
        from core.pipelines.ocr.credit_experiment import PaddleOcrEngine
        paddle_engine = PaddleOcrEngine()
        print(f"[BoxTracking] PaddleOCR hazır ({perf_counter()-t0:.1f}s). Device: {paddle_engine.device}")
    except Exception as exc:
        print(f"[HATA] PaddleOCR başlatılamadı: {exc}", file=sys.stderr)
        sys.exit(1)

    # --- Box motion track modülünü yükle ---
    from core.pipelines.ocr.boxtracking.box_motion_track import run_box_motion_track
    ...
    print(f"\n[BoxTracking] Özet: {summary_path}")
    print(f"[BoxTracking] Çıktı: {output_base}")
