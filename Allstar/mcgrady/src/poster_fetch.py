...
McGrady KULE KOPYASI (2026-08-18, OCR-worktree/pdf-mitas/poster_fetch.py).
Tek fark: scripts/'e geri sapan sys.path hack'i KESİLDİ — kule-içi web_cache
aynı dizinde, düz import yeterli. Kalan mantık orijinalle birebir.
...
# baslik ("Ahlat Agaci") AYNI norm'a duser.
...
# URL-önbellek (hız #2, 2026-07-05): karar mantığı DEĞİŞMEZ — yalnız HTTP yanıtı
# diskten döner. KULE KOPYASI: web_cache aynı dizinde (src/) — path hack yok.
try:
...
# Karayip Korsanları 1↔2 gibi sekel karışmasını keser.
...
def _tmdb_cand_has_name(rec: dict, kind: str, key: str, names_norm: list) -> bool:
    ...
    KADRO-ZORUNLU KAPI (id-güvenilir zincirin kör title-search dalına taşındı —
    2026-07-09, REKABET/Challengers kökü): id-tabanlı doğrudan lookup (tmdb_id /
    imdb_id → /find) zaten doğrulanmış kabul edilir ve bu kapıya TABİ DEĞİL; ama o
    dallar posterless dönüp KÖR başlık-metni aramasına düştüğünde, aday KENDİ
    kadrosuyla teyit edilmeden kabul edilemez. names_norm boşsa (kadro hiç
    bilinmiyor) GÜVENSİZ → False."""
    if not names_norm:
        return False
    try:
        cr = json.loads(_get("https://api.themoviedb.org/3/%s/%s/credits?api_key=%s" % (kind, rec.get("id"), key)))
    except Exception:
        return False
    cand = {"s": " ".join(
...
def _fetch_tmdb_poster(tmdb_id=None, imdb_id=None, title=None, original=None, year=None, vsig=None,
    ...
    def _data(path):
        if not path:
            return None
        try:
            return _get("https://image.tmdb.org/t/p/w780%s" % path, binary=True)
        except Exception:
            return None
    ...
    try:
        if tmdb_id:
            meta = json.loads(_get("https://api.themoviedb.org/3/movie/%s?api_key=%s" % (tmdb_id, key)))
            if _ver_ok_rec(meta):
                d = _data(meta.get("poster_path"))
                if d:
                    return d
        if imdb_id:                                  # IMDb id → /find (DOĞRU endpoint; /movie/{tt} 404 verir)
            fr = json.loads(_get("https://api.themoviedb.org/3/find/%s?api_key=%s&external_source=imdb_id" % (imdb_id, key)))
        ...
        for q in (original, title):                  # başlık araması (yabancı orijinal önce)
            ...
            for kind in ("movie", "tv"):             # FİLM ve DİZİ ara (Diriliş Ertuğrul = tv)
                url = ("https://api.themoviedb.org/3/search/%s?api_key=%s"
                       "&query=%s&include_adult=false" % (kind, key, urllib.parse.quote(q)))
                if year:
                    url += ("&year=%s" % year) if kind == "movie" else ("&first_air_date_year=%s" % year)
...
def _omdb_cand_has_name(meta: dict, names_norm: list) -> bool:
    ...
    if not names_norm:
        return False
    cand = {"s": "%s %s" % (meta.get("Actors") or "", meta.get("Director") or "")}
    return _cand_has_name(cand, names_norm)
...
def _fetch_omdb_poster(imdb_id=None, title=None, year=None, names_norm=None) -> bytes | None:
    ...
    (t=) + yıl — KÖR metin araması, names_norm verilirse KADRO-ZORUNLU KAPI uygulanır."""
    import os
    ...
    urls = []
    if imdb_id:
        urls.append(("id", "http://www.omdbapi.com/?i=%s&apikey=%s" % (imdb_id, key)))
    if title:
        u = "http://www.omdbapi.com/?t=%s&apikey=%s" % (urllib.parse.quote(title), key)
        if year:
            u += "&y=%s" % year
        urls.append(("title", u))
    for kind, u in urls:
    ...
    for lang, t in titles_to_try:
        ...
        try:
            ...
            url = ("https://%s.wikipedia.org/w/api.php?action=query&prop=pageimages"
                   "&format=json&piprop=original&titles=%s" % (lang, enc))
...
def fetch_poster(title: str, out_path, *, original: str | None = None,
    ...
    # Aday sorgular: orijinal ad (yabancı film) ve TRT/Türkçe ad; her birinin
    # hem HAM hem TEMİZLENMİŞ hali denenir. _search exact-norm eşleşmesi
    # değişmediği için yanlış afiş riski ARTMAZ.
    queries = []
    ...
    # KADRO-TEYİTLİ KAPI (KESİN İLKKE 4): id-tabanlı dış aramalar (tmdb_id / imdb_id → TMDB /find)
...
def _search(query: str, year=None, names_norm=None, vsig=None):
    ...
    kökü, 2026-07-09): ünlü kişi/ikili adına benzeyen katalog başlıklarında sorgu METNİ zaten
    o kişinin adı olduğundan, başlık-eşleşmesi VE kadro-kapısı AYNI kişiden türer (bağımsız kanıt
    değil) → kişi HAKKINDA sonraki-dönem anma/derleme yapımları kadro-kapısını yanlışlıkla geçebilir."""
    ...
    try:
        d = json.loads(_get("https://v3.sg.media-imdb.com/suggestion/%s/%s.json" % (fc, urllib.parse.quote(query)))).get("d", [])
    except Exception:  # noqa: BLE001
        return None
    tt = [x for x in d if str(x.get("id", "")).startswith("tt") and (x.get("i") or {}).get("imageUrl")]
    # VERSİYON AYRACI: okunan başlıkta sekel imzası varsa, aday başlığı o versiyonla
    # tutarsızsa ELE. İmza yoksa _version_ok hep True → atıl.
    if vsig and vsig.get("active"):
        tt = [x for x in tt if _version_ok(vsig, x.get("l"))]
        if not tt:
            return None
    # ERA-SANITY (2026-07-09): TRT bir filmi kataloglamışsa aday IMDb'nin kendi yıl
    # bilgisiyle katalog-yılından >2 yıl YENİ olamaz. year yoksa veya adayda 'y' yoksa
    # dokunulmaz (fail-open; tek kanıt değil, ek emniyet katmanı).
    import os
    ...
    if not cands:
        return None
    # Tam başlık eşleşmesi → kural olarak güvenli tek aday. ANCAK yabancı-başlık
    # tuzağı: gerçek film IMDb'de farklı başlıkla kayıtlıyken aynı adı taşıyan
    # ALAKASIZ yapım tek-exact kazanabiliyordu. Kadro biliniyor + exact adayın 's'
    # alanı dolu ama kadrodan kimse geçmiyorsa → kadrosu TUTAN adayı tercih et;
    # net değilse yanlış afiş yerine None.
    ...
    if names_norm:                                                         # ASIL ayraç: kadro teyidi
        ...
        if len(named) == 1:
            return named[0]
    # YIL-TABANLI VERSİYON SEÇİMİ KALDIRILDI (Çağatay 2026-06-14): tek adaya
    # KADRO/VERSİYON ile inemiyorsak → afiş YOK (şans ile yürümez).
    return None
...
if __name__ == "__main__":
    ...
    # KULE NOTU: orijinaldeki E:\MITAS test yolu → kule-içi göreli yol.
    p = fetch_poster(t, "_afis_test.jpg", cast=["Kate Winslet", "Mackenzie Foy"])
