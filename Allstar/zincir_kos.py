...
def klip_kes(film_id: str, bolum: str, video: Path | None, logf) -> Path | None:
    ...
    try:
        ...
        bit = float(d.get("bitis_sn") or 0.0)
    except Exception:                                     # noqa: BLE001
        return None
    if bit <= bas:
        return None
    bas_kes = max(0.0, bas - GERI_PAY_SN)
    sure = (bit - bas_kes) + GERI_PAY_SN
    ...
    komut = ["ffmpeg", "-y", "-v", "error", "-ss", f"{bas_kes:.3f}",
             "-t", f"{sure:.3f}", "-i", str(video), "-an", "-c", "copy",
             str(hedef)]
    try:
    ...
    if r.returncode != 0 or not hedef.is_file() or hedef.stat().st_size == 0:
        ...
        return None
    log(f"    klip: {bas_kes:.0f}-{bas_kes+sure:.0f} sn ({hedef.stat().st_size//1024} KB)", logf)
    return hedef
