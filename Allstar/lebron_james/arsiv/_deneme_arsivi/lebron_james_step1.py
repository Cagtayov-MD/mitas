...
def get_ocr_engine():
    global _ocr_engine
    # TEMP: Force fallback for testing (PaddleOCR 3.7.0 predict() bug)
    return None
...
def compose_lebron(
    ...
    def _gecerli_segment(si, so, idx):
        if not si:
            return False
        if len(si) > 1:
            return True
    ...
    parcalar = [_segment_kanvas(si, so) for si, so, idx in segmentler if _gecerli_segment(si, so, idx)]
    if not parcalar:
        parcalar = [_segment_kanvas(si, so) for si, so, idx in segmentler if si]
    if not parcalar:
    ...
    manifest = {
        ...
        "segment_kareler": [list(idx) for _, _, idx in segmentler if idx],
