...
Kimlik KESİNLEŞEN (cast/kimlik kilitli) filmlerde KB-gerçek yönetmen+cast biliniyor.
...
Çalıştır: python outputs/yanlis_okuma_bul.py [N]
"""
...
from credit_text_read import _valid_person_name, _looks_garble
...
def main():
    ...
    try:
        for folder in folders:
            ...
            title, year = ab._parse_folder(base)
            if not title:
                continue
            ...
            olculen += 1
            try:
                res = q.qc_credit_block(yon, cast, yap, title=title, year=year, ozet=ozet, kb=kb)
            except Exception:
                continue
            if not res["kimlik"]["locked"]:
                continue
            kilitli += 1
            real_yon = res.get("otoriter_yon") or []
            real_cast = res.get("otoriter_cast") or []
            ocr_dir = (yon[0].strip() if yon and yon[0].strip() else None)
            # ── YÖNETMEN yanlış-okuma sınıflaması (gerçek biliniyorsa) ──
            if real_yon:
                if not ocr_dir:
                    ...
                    print(f"  [ÇÖP-YÖN] {title} ({year}): OCR='{ocr_dir[:40]}' → GERÇEK={real_yon[0]}", flush=True)
                else:
                    yanlis_yon.append((title, year, ocr_dir, real_yon[0]))
                    print(f"  [YANLIŞ-YÖN] {title} ({year}): OCR='{ocr_dir}' → GERÇEK='{real_yon[0]}'", flush=True)
            # ── CAST garble/yanlış: OCR cast'inde KB-gerçeğe eşleşmeyen kişi-isimleri ──
            if real_cast:
