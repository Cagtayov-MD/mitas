#!/bin/bash
# ÖZET DÖNGÜSÜ — cron nöbetçili, her turda PDF basar (ilerleme kalıcı).
cd /opt/mitas || exit 1
...
PY
  N=$(tr ',' '\n' < $S/hedef.txt | grep -c .)
  echo "TUR $tur: $N özetsiz $(date '+%T')"
  [ "$N" -lt 5 ] && break
  venvs/ocr/bin/python kurulum/32_ozet_web.py --films-file "$S/hedef.txt" --force --workers 6 > "$H/_tur.log" 2>&1
  # HER TURDAN SONRA: künyeye işle + PDF bas (ilerleme kalıcı)
...
PY
  venvs/ocr/bin/python - > $S/basilacak.txt <<'PY'
import json,glob,os
...
PY
  venvs/asr/bin/python kurulum/26_kapanis_pdf.py --films "$(cat $S/basilacak.txt)" --no-net 2>&1 | tail -1
  venvs/ocr/bin/python - <<'PY'
...
print("  TAM PDF:",len(glob.glob(E+"/*.pdf")))
PY
done
echo "DÖNGÜ BİTTİ $(date '+%T')"
touch "$H/_OZET_BITTI"
