#!/bin/bash
# SON MİKRO-GEÇİŞ: 2 kalan aday, doğru ayarlarla.
#  1) Qwen3-VL-30B-AWQ: 30s klipler + max_pixels(125440) + 12k  → video ~7.2k token ✓
#  2) Qwen2.5-Omni-7B-AWQ: 60s + 32k (AWQ hafif, KV bol)        → video ~30.7k token ✓
set -uo pipefail
LOG=/opt/mitas/kurulum/logs/19b_mikro.log
exec > >(tee "$LOG") 2>&1
...
oku() { local SNAP="$1" AD="$2" MP4="$3" MAXLEN="$4" EKSTRA="$5"
  echo "===== $AD ($MP4, len=$MAXLEN) $(date +%H:%M) ====="
  ...
  if [ "$OK" = "0" ]; then
    echo "$AD: SUNUCU-KALKMADI"; echo -e "\n## $AD\nSUNUCU KALKMADI" >> "$RAPOR"; kill $SRV 2>/dev/null; sleep 5; return
  fi
...
S30=$(ls -d $H/models--QuantTrio--Qwen3-VL-30B-A3B-Instruct-AWQ/snapshots/*/ | head -1)
oku "$S30" "Qwen3-VL-30B-AWQ-30sA" /opt/mitas/outputs/jenerik_rol_a.mp4 12288 "$QW"
oku "$S30" "Qwen3-VL-30B-AWQ-30sB" /opt/mitas/outputs/jenerik_rol_b.mp4 12288 "$QW"

export HF_HOME=/opt/mitas/models/QwenOmni/hf-cache
SOM=$(ls -d /opt/mitas/models/QwenOmni/hf-cache/hub/models--Qwen--Qwen2.5-Omni-7B-AWQ/snapshots/*/ | head -1)
oku "$SOM" "Qwen2.5-Omni-7B-AWQ-32k" /opt/mitas/outputs/jenerik_rol_test.mp4 32768 ""
echo "MIKRO-GECIS-TAMAM $(date)"
