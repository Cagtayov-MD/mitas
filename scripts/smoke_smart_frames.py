...
# -*- coding: utf-8 -*-
"""
smoke_smart_frames.py
Akıllı frame seçimi: son 60 cikis karesi (jenerik orada) + ilk 15 giris karesi (studio logo).
gemma3:12b vs gemma4:26b vs glm-ocr karşılaştırması.
"""
import base64, glob, io, json, os, re, time, unicodedata, urllib.request
...
GOLD = {
    ...
    "YALAZA": {
        "yonetmen": [],
        "oyuncular": []  # Türkçe doğruluk testi
    }
}
...
def get_frames_smart(folders_strategy, budget_per_folder=10):
    """Son N kare (cikis_last) veya ilk N kare (giris_first) stratejisi."""
    frames = []
    for key, (folder, window) in folders_strategy.items():
        ...
        if "last" in key:
            ...
            pool = pngs[:window]
        # Budget: her havuzdan eşit dağılım
        step = max(1, len(pool) // budget_per_folder)
        frames.extend(pool[::step][:budget_per_folder])
    return frames
...
def call_ollama(model, frames, prompt, timeout=300):
    images = []
    for f in frames:
        try:
    ...
    payload = json.dumps({
        ...
        "stream": False,
        "options": {"num_ctx":8192,"num_predict":700,"temperature":0,"think":False}
    }).encode("utf-8")
...
MODELS = [
    ("gemma3:12b",  "gemma3:12b (YENİ, 8.1GB, multimodal)"),
...
def main():
    ...
    for model_tag, model_label in MODELS:
        ...
        for film in TEST_FILMS:
            frames = get_frames_smart(film["folders_strategy"], budget_per_folder=10)
            if not frames:
                print(f"  {film['id']}: KARE YOK")
                model_res["filmler"].append({"id":film["id"],"durum":"KARE_YOK"})
                continue

            print(f"\n  [{film['id']}] {len(frames)} kare (akıllı seçim)...", flush=True)
            text, elapsed = call_ollama(model_tag, frames, PROMPT)
    ...
    # Kaydet
    out_j = os.path.join(OUTPUT_DIR, "smart_smoke.json")
    with open(out_j, "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)

    out_t = os.path.join(OUTPUT_DIR, "smart_smoke.txt")
    lines = ["MITAS AKILLI FRAME SMOKE TEST", "="*60, ""]
    for r in all_results:
        ...
        for film in r.get("filmler",[]):
            if "hit_oyuncu" in film:
                ...
                lines.append(f"    RAW: {film['cikti'][:400]}")
        lines.append("")
