...
ERD_MEDIA = ROOT / "testklipler" / "erd_test_video.mp4"
...
class AsrTranscribeRealMediaTest(unittest.TestCase):
    ...
    def test_transcribe_real_media_vad_segments_smoke(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            selected_vad_segments = vad_result.speech_segments[:4]
        ...
        self.assertIn("hazırlık", result.transcript.lower())
        self.assertGreater(sum(result.language_distribution.values()), 0)

        first_start = max(0.0, min(segment.start for segment in selected_vad_segments) - result.chunk_padding_seconds)
        last_end = max(segment.end for segment in selected_vad_segments) + result.chunk_padding_seconds
    ...
    @unittest.skipUnless(_runtime_available(), "ffmpeg, ffprobe, silero_vad, faster_whisper, and local large-v3 are required")
    def test_transcribe_erd_real_media_multilingual_smoke(self) -> None:
        if not ERD_MEDIA.exists():
            self.skipTest(f"real media fixture is not available: {ERD_MEDIA}")

        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            normalized = normalize_audio(ERD_MEDIA, output_dir=temp_path / "normalized")
            vad_result = run_silero_vad(normalized.output_path)
            selected_vad_segments = vad_result.speech_segments[38:44]
            result = transcribe_vad_segments(
                normalized.output_path,
                selected_vad_segments,
                chunk_output_dir=temp_path / "chunks",
        ...
        self.assertEqual(result.vad_segments_count, len(selected_vad_segments))
        self.assertTrue(result.multilingual)
        self.assertGreaterEqual(result.language_distribution.get("tr", 0), 1)
        self.assertGreaterEqual(result.language_distribution.get("en", 0), 1)
        self.assertIn("Prime Minister", result.transcript)
        self.assertIn("don't have time", result.transcript)
        self.assertIn("Altıncı", result.transcript)
